...
gulp.task('test:process_payments', ['build:node'], function() {
  return gulp.src(["test/test-helper.js", "test/unit/process_payments_test.js"])
    .pipe(plugins.mocha({
      reporter: ['spec']
    }));
});

gulp.task('test:make_payment', ['build:node'], function() {
  return gulp.src(["test/test-helper.js", "test/unit/make_payment_test.js"])
    .pipe(plugins.mocha({
      reporter: ['spec']
    }));
});

gulp.task('test:payments', ['build:node'], function() {
  return gulp.src(["test/test-helper.js", "payments/test/*.js"])
    .pipe(plugins.mocha({
      reporter: ['spec']
    }));
});

gulp.task('test:process_payment', ['build:node'], function() {
  return gulp.src(["test/test-helper.js", "payments/test/process_payment_test.js"])
    .pipe(plugins.mocha({
      reporter: ['spec']
    }));
});

gulp.task('test:multi_sig_payment', ['build:node'], function() {
  return gulp.src(["test/test-helper.js", "payments/test/multi_sig_payment_test.js"])
    .pipe(plugins.mocha({
      reporter: ['spec']
    }));
});
